...
public class PinnedHeaderExpandableListView extends ExpandableListView implements OnScrollListener, OnGroupClickListener {
    ...
    private int indexPosition = -1;     //判断这个ExpandableListView的滑动方向所需要的临时比较变量
    private int groupPosition = -1;
    private static final int UP_SCROLL = 1;     //上滑用1表示
    private static final int DOWN_SCROLL = 2;   //下滑用2表示
    ...
    private void headerViewClick() {
        ...
        if (mAdapter.getGroupClickStatus(groupPosition) == 1) {
            expandOrCollapseGroup(this, groupPosition);
    ...
    public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition, long id) {
        ...
        if (mAdapter.getGroupClickStatus(groupPosition) == 0) {// 点击需展开
            mAdapter.setGroupClickStatus(groupPosition, 1);
            expandOrCollapseGroup(parent, groupPosition);
    ...
    /**
     * 关闭或者展开ExpandableListView
     *
     * @param exListView    ExpandableListView
     * @param groupPosition group位置
     */
    private void expandOrCollapseGroup(ExpandableListView exListView, int groupPosition) {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        judgeScrollOrientation(scrollState);    //判断滑动状态
    }

    /**
     * 判断这个PinnedHeaderExpandableListView的滑动方向
     *
     * @param scrollState 滑动的状态
     */
    private void judgeScrollOrientation(int scrollState) {
        switch (scrollState) {
            //滚动之前，手还在屏幕上，记录滚动钱的下标
            case SCROLL_STATE_TOUCH_SCROLL:
                indexPosition = getLastVisiblePosition();
                break;

            //停止滚动
            case SCROLL_STATE_IDLE:
                int scrollPosition = getLastVisiblePosition();
                if (scrollPosition > indexPosition) {
                    Log.i(TAG, "上");
                    /**向上滑动*/
                    if (null != onRelateLeft) {
                        onRelateLeft.relateLeft(this.groupPosition, UP_SCROLL);
                    }
                } else {
                    Log.i(TAG, "下");
                    /**向下滑动*/
                    if (null != onRelateLeft) {
                        onRelateLeft.relateLeft(this.groupPosition, DOWN_SCROLL);
                    }
                }
                break;
        }
    ...
    private OnRelateLeft onRelateLeft;

    public void setOnRelateLeft(OnRelateLeft onRelateLeft) {
        this.onRelateLeft = onRelateLeft;
    }

    public interface OnRelateLeft {
        void relateLeft(int groupPosition, int orientation);
    }
}
